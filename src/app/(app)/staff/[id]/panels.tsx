  ...
  return (
    <form action={submit} key={pending ? "p" : "i"}>
      ...
      <div className="grid gap-3 md:grid-cols-6">
        ...
        <Field label="Completed on" error={e.completedOn} className="md:col-span-2"><DateInput name="completedOn" required /></Field>
        <Field label="Expires on" error={e.expiresOn} hint="Certificates and licenses" className="md:col-span-2"><DateInput name="expiresOn" /></Field>
...
export function AssignmentPanel({ staffId, assignments, candidates }: { staffId: string; assignments: AssignmentRow[]; candidates: { id: string; name: string }[] }) {
  ...
  return (
    <div>
      {active.length === 0 ? <p className="px-5 py-4 text-[13px] text-muted-foreground">No clients assigned. This caregiver cannot clock in until someone is assigned.</p> : (
...
export function LoginPanel({ staffId, login, defaultEmail, isSelf }: { staffId: string; login: LoginInfo | null; defaultEmail: string; isSelf: boolean }) {
  ...
  if (!login) {
    ...
    return (
      <form action={create} className="space-y-3">
        <p className="text-[13px] text-muted-foreground">No login yet. Create one so this person can clock in.</p>
  ...
  return (
    <form action={update} className="space-y-3">
      ...
      <label className="flex items-center gap-2 text-[13px]"><input type="checkbox" name="active" defaultChecked={login.active} disabled={isSelf} className="h-4 w-4 accent-[var(--primary)]" /> Login enabled</label>
