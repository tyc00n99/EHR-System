        ...
        <div className="space-y-4">
          ...
          <Card title="Training and credentials" description="Add each certificate, clearance, or training as it is completed">
            {credentials.length === 0 ? <Empty icon="doc" title="Nothing recorded yet" /> : (
              <Table><Thead><Th>Item</Th><Th>Completed</Th><Th>Expires</Th><Th align="right">Hours</Th><Th>Note</Th><Th /></Thead><tbody>{credentials.map((c) => <Tr key={c.id}><Td strong>{c.title}<div className="text-[13px] font-normal text-muted-foreground">{credentialLabel(c.type)}</div></Td><Td className="text-muted-foreground">{fmtDate(c.completedOn)}</Td><Td className="text-muted-foreground">{c.expiresOn ? fmtDate(c.expiresOn) : "—"}</Td><Td align="right">{c.hours ?? ""}</Td><Td wrap className="text-[13px] text-muted-foreground">{c.note}</Td><Td align="right"><DeleteCredential id={c.id} staffId={id} /></Td></Tr>)}</tbody></Table>
            )}
