      ...
      <div className="grid gap-4 lg:grid-cols-[1fr_360px]">
        <Card title="Ready to bill" description="Grouped by client. One row per visit becomes one 837P service line." actions={<Badge tone="ok">{ready.length} lines</Badge>}>
          {byClient.length === 0 ? <Empty icon="money" title="Nothing ready in this period" /> : byClient.map((c) => (
            <div key={c.personId} className="border-b border-line-soft last:border-b-0">
              <div className="flex items-center justify-between bg-sidebar px-5 py-1.5 text-[12px]"><Link href={`/clients/${c.personId}`} className="font-semibold text-text-strong hover:underline">{c.name}</Link><span className="tabular-nums text-muted">{c.lines.length} line{c.lines.length === 1 ? "" : "s"} · {fmtMoney(total(c.lines))}</span></div>
              <Table>
                ...
                <tbody>{c.lines.map((l) => <Tr key={l.visitId}><Td><Link href={`/visits/${l.visitId}`} className="hover:underline">{labelForCode(l.serviceCode, [])}</Link></Td><Td className="tabular-nums">{l.serviceCode}</Td><Td align="right">{l.units}</Td><Td align="right">{fmtMoney(l.unitRate)}</Td><Td align="right" strong>{fmtMoney(l.units * l.unitRate)}</Td><Td className="text-muted">{l.staffName}</Td></Tr>)}</tbody>
              </Table>
            </div>
          ))}
        </Card>
        <div className="space-y-4">
          <Card title="On hold" description="Fix these before the claim run" actions={<Badge tone={hold.length ? "warn" : "ok"}>{hold.length}</Badge>}>
            {hold.length === 0 ? <p className="px-5 py-4 text-[13px] text-muted">Nothing held.</p> : (
              <ul className="divide-y divide-line-soft">{hold.map((l) => <li key={l.visitId}><Link href={`/visits/${l.visitId}`} className="flex items-center justify-between gap-3 px-5 py-2.5 hover:bg-hover"><span className="min-w-0"><span className="block truncate font-medium text-text-strong">{l.personName}</span><span className="block text-[12.5px] text-muted">{!l.signed ? "Not signed by client" : "Manual entry pending EVV"} · {fmtMoney(l.units * l.unitRate)}</span></span><Badge tone={!l.signed ? "danger" : "warn"}>{!l.signed ? "unsigned" : "manual"}</Badge></Link></li>)}</ul>
            )}
          </Card>
          <Card title="Claim run" padded>
            <p className="text-[13px] text-muted">837P generation and the MN-ITS submission are the next milestone. The lines above are already shaped for it: provider tax ID, PMI, HCPCS with modifiers, units, rendering NPI or UMPI, and dates of service.</p>
