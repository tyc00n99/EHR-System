        ...
        return (
          <section key={g.id} className={cx("rounded-lg border border-line bg-card shadow-[var(--shadow-sm)]", g.status !== "active" && "opacity-70")}>
            <div className="flex items-start gap-3 border-b border-line-soft px-5 py-4">
              ...
              {manage && (
                <div className="w-36 shrink-0">
                  <Select value={g.status} onChange={(e) => start(async () => { await setGoalStatus(g.id, personId, e.target.value as GoalView["status"]); toast.success("Goal updated"); })} disabled={pending} className="h-8 text-[13px]">
            ...
            <div className="px-5 py-4">
              <div className="mb-2 text-[13px] font-medium uppercase tracking-[0.06em] text-muted-foreground">Questions · responses {rangeLabel}</div>
              <ol className="space-y-4">
                {g.questions.map((q, i) => {
                  const total = q.yes + q.no;
                  return (
                    <li key={q.id}>
                      <div className="mb-2 flex items-center gap-2 text-[13.5px]"><span className="flex h-6 w-6 shrink-0 items-center justify-center rounded-full bg-panel text-[13px] font-semibold text-text-strong">{i + 1}</span><span className="flex-1">{q.prompt}</span>{q.na > 0 && <span className="text-[13px] text-muted-foreground">{q.na} n/a</span>}{manage && <button disabled={pending} onClick={() => { if (confirm("Retire this question? Past responses stay.")) start(() => retireGoalQuestion(q.id, personId)); }} className="text-muted-foreground hover:text-danger"><Trash2 className="size-3.5" /></button>}</div>
              ...
              {manage && g.status === "active" && (
                <div className="mt-4 flex gap-2 sm:pl-8">
                  <Input value={newQ[g.id] ?? ""} onChange={(e) => setNewQ((s) => ({ ...s, [g.id]: e.target.value }))} placeholder="Add a yes/no question caregivers answer on each note" className="h-8 text-[13px]" />
    ...
    <form action={submit} key={pending ? "p" : String(state.message ?? "")} className="rounded-lg border border-line bg-sidebar p-5">
      ...
      <div className="grid gap-3 md:grid-cols-6">
        ...
        <div className="md:col-span-6">
          ...
          <button type="button" onClick={() => setQuestions((qs) => [...qs, ""])} className="mt-2 text-[13px] font-medium text-primary hover:underline">+ Another question</button>
        </div>
      </div>
