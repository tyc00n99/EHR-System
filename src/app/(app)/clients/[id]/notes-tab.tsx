  ...
  return (
    <div className="mx-auto max-w-4xl">
      <form action={base} className="mb-4 flex flex-wrap items-end gap-3 rounded-xl border border-line bg-sidebar px-4 py-3">
        ...
        <label className="block"><span className="mb-1 block font-mono text-[13px] font-medium uppercase tracking-[0.06em] text-muted-foreground">Service</span><select name="code" defaultValue={filters.code} className="h-9 rounded-lg border border-line bg-page px-2.5 text-[13px]"><option value="">All services</option>{codes.map((c) => <option key={c.code} value={c.code}>{c.label} · {c.code}</option>)}</select></label>
        <label className="block"><span className="mb-1 block font-mono text-[13px] font-medium uppercase tracking-[0.06em] text-muted-foreground">From</span><input name="from" type="date" defaultValue={filters.from} className="h-9 rounded-lg border border-line bg-page px-2.5 text-[13px]" /></label>
        <label className="block"><span className="mb-1 block font-mono text-[13px] font-medium uppercase tracking-[0.06em] text-muted-foreground">To</span><input name="to" type="date" defaultValue={filters.to} className="h-9 rounded-lg border border-line bg-page px-2.5 text-[13px]" /></label>
        ...
        <div className="ml-auto flex items-center gap-3">
          <span className="text-[13px] tabular-nums text-muted-foreground">{rows.length} notes · {units} units</span>
      ...
      <div className="space-y-3">
        {rows.map((r) => (
          <Link key={r.id} href={`${base}?tab=notes&${q}&visit=${r.id}`} scroll={false} className="block overflow-hidden rounded-xl border border-line bg-card shadow-[var(--shadow-sm)] transition-colors hover:bg-hover">
            <div className="flex flex-wrap items-center gap-x-3 gap-y-1 border-b border-line-soft bg-sidebar px-4 py-2 text-[13px]">
            ...
            <div className="grid gap-4 px-4 py-3 md:grid-cols-[1fr_200px]">
              <div>
                <div className="font-mono text-[13px] font-medium uppercase tracking-[0.06em] text-muted-foreground">Progress review</div>
                <p className={cx("mt-1 leading-6", r.note ? "text-text" : "text-hint")}>{r.note ?? "No note yet."}</p>
              </div>
              <dl className="space-y-1.5 text-[13px]">
                <div><dt className="font-mono text-[13px] font-medium uppercase tracking-[0.06em] text-muted-foreground">Service</dt><dd>{labelForCode(r.serviceCode, r.modifiers)}</dd></div>
                {r.interaction && <div><dt className="font-mono text-[13px] font-medium uppercase tracking-[0.06em] text-muted-foreground">Interaction</dt><dd className="capitalize">{r.interaction}</dd></div>}
                {r.skills.length > 0 && <div><dt className="font-mono text-[13px] font-medium uppercase tracking-[0.06em] text-muted-foreground">Skills</dt><dd>{r.skills.join(", ")}</dd></div>}
                {r.goalYes + r.goalNo > 0 && <div><dt className="font-mono text-[13px] font-medium uppercase tracking-[0.06em] text-muted-foreground">Goals</dt><dd>{r.goalYes} yes · {r.goalNo} no</dd></div>}
              </dl>
            </div>
          </Link>
        ))}
      </div>
      {rows.length > 0 && <p className="mt-3 text-[13px] text-muted-foreground">Showing {fmtDate(rows[rows.length - 1].clockInAt)} to {fmtDate(rows[0].clockInAt)}. Click a note to open the full record.</p>}
    </div>
  );
}
