...
export function PreviewButton({ href, title, mime, variant = "pill" }: { href: string; title: string; mime: string; variant?: "pill" | "icon" }) {
  ...
  if (!previewable) {
    if (variant === "icon") return <a href={href} target="_blank" rel="noreferrer" aria-label={`Open ${title}`} title="Open" className="flex size-7 shrink-0 items-center justify-center rounded-md border border-line bg-card hover:bg-hover"><FileText className="size-4" /></a>;
    return null;
  }
  return (
    <>
      {variant === "icon"
        ? <button type="button" onClick={() => setOpen(true)} aria-label={`Open ${title}`} title="Open" className="flex size-7 shrink-0 items-center justify-center rounded-md border border-line bg-card hover:bg-hover"><FileText className="size-4" /></button>
