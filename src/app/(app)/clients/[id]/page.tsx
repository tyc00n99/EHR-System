...
import { Badge, Card, Crumb, CrumbSep, Empty, LinkButton, Properties, RecordHeader, Table, Tabs, Td, Th, Thead, Tr, cx } from "@/components/ui";
import { canViewPerson, getPerson, listAgreementsForPerson, listAssignmentsForPerson, listAuditForRecord, listClientDocuments, listVisits } from "@/db/queries";
...
function Contact({ title, name, sub, phone, email }: { title: string; name: string | null; sub?: string | null; phone?: string | null; email?: string | null }) {
  return (
    <Card title={title} padded>
      {name ? (<><div className="font-medium text-text-strong">{name}{sub && <span className="font-normal text-muted"> · {sub}</span>}</div><div className="mt-1.5 flex flex-col gap-1 text-[13px]">{phone && <a href={`tel:${phone}`} className="flex items-center gap-1.5 text-accent hover:underline"><Icon.phone size={13} />{phone}</a>}{email && <a href={`mailto:${email}`} className="flex items-center gap-1.5 text-accent hover:underline"><Icon.mail size={13} />{email}</a>}{!phone && !email && <span className="text-hint">No contact details</span>}</div></>) : <div className="text-[13px] text-muted">{title === "Guardian" ? "None on file. The person is their own legal representative." : "None on file."}</div>}
    </Card>
  );
}
...
export default async function ClientPage({ params, searchParams }: PageProps<"/clients/[id]">) {
  ...
  const [agreements, visits, audit, documents, team] = await Promise.all([
    ...
    <div>
          ...
          <div className="space-y-4">
            <Card title="Profile" padded>
              <Properties labelWidth={104} items={[
                ...
                { icon: "mail", label: "Email", value: person.email ? <a href={`mailto:${person.email}`} className="text-accent hover:underline">{person.email}</a> : null },
            ...
            <Card title={track ? `Planning deadlines · ${track}` : "Planning deadlines"} padded>
              {deadlines.length === 0 ? <p className="text-[13px] text-muted">{person.serviceStartDate ? "Add a service agreement with a program to compute deadlines." : "Set a service start date to compute deadlines."}</p> : (
                <ul className="space-y-2.5">{deadlines.map((d) => { const overdue = d.due < new Date(); return <li key={d.id} className="text-[13px]"><div className={cx("font-medium tabular-nums", overdue ? "text-danger" : "text-text-strong")}>{fmtDate(d.due)}{overdue && <span className="ml-1.5 font-normal">overdue</span>}</div><div className="text-text">{d.label}</div><div className="text-xs text-muted">{d.cite}</div></li>; })}</ul>
              )}
            </Card>
          </div>
          <div className="space-y-4">
            <div className="grid gap-3 sm:grid-cols-3">
              <div className="rounded-lg border border-line bg-card px-4 py-3 shadow-[var(--shadow-sm)]"><div className="text-[12.5px] font-medium text-muted">Units remaining</div><div className="mt-1 text-[22px] font-bold tabular-nums text-text-strong">{unitsLeft.toLocaleString()}</div><div className="text-[12.5px] text-muted">across {active.length} active authorization{active.length === 1 ? "" : "s"}</div></div>
              <div className="rounded-lg border border-line bg-card px-4 py-3 shadow-[var(--shadow-sm)]"><div className="text-[12.5px] font-medium text-muted">This pay period</div><div className="mt-1 text-[22px] font-bold tabular-nums text-text-strong">{periodVisits.reduce((n, r) => n + r.visit.units, 0)} <span className="text-[13px] font-normal text-muted">units</span></div><div className="text-[12.5px] text-muted">{periodVisits.length} completed visit{periodVisits.length === 1 ? "" : "s"}</div></div>
              <div className={cx("rounded-lg border bg-card px-4 py-3 shadow-[var(--shadow-sm)]", unsigned ? "border-danger/30" : "border-line")}><div className="text-[12.5px] font-medium text-muted">Unsigned visits</div><div className={cx("mt-1 text-[22px] font-bold tabular-nums", unsigned ? "text-danger" : "text-text-strong")}>{unsigned}</div><div className="text-[12.5px] text-muted">in the periods shown</div></div>
            </div>
            <Card title="Authorizations" actions={<Link href={`/clients/${id}?tab=authorizations`} className="text-[13px] font-medium text-accent hover:underline">Manage</Link>}>
              {active.length === 0 ? <Empty icon="doc" title="No active authorization" action={manage && <LinkButton href={`/clients/${id}/agreements/new`} variant="primary">Add an agreement</LinkButton>}>Visits cannot be recorded until one exists.</Empty> : (
                <ul className="divide-y divide-line-soft">{active.map(({ agreement: a, unitsUsed }) => <li key={a.id} className="flex items-center gap-3 px-5 py-3"><Ring used={unitsUsed} total={a.authorizedUnits} size={36} /><div className="min-w-0 flex-1"><div className="truncate font-medium text-text-strong">{labelForCode(a.serviceCode, a.modifiers)}</div><div className="text-[12.5px] text-muted tabular-nums">{a.serviceCode} {a.modifiers.join(" ")} · {(a.authorizedUnits - unitsUsed).toLocaleString()} of {a.authorizedUnits.toLocaleString()} units left · through {fmtDate(a.endDate)}</div></div><span className="text-[13px] tabular-nums text-muted">{fmtMoney(a.unitRate)}/unit</span></li>)}</ul>
              )}
            </Card>
            <Card title="Recent visits" actions={<Link href={`/clients/${id}?tab=visits`} className="text-[13px] font-medium text-accent hover:underline">All visits</Link>}>
        ...
        <Card title="Service agreements" description="Every authorization on file, current and past" actions={manage && <LinkButton href={`/clients/${id}/agreements/new`} variant="primary">New agreement</LinkButton>}>
          {agreements.length === 0 ? <Empty icon="doc" title="No service agreements yet" /> : (
            <Table>
              ...
              <tbody>{agreements.map(({ agreement: a, unitsUsed }) => <Tr key={a.id} muted={a.status !== "active"}><Td strong>{a.agreementNumber}{a.documentPath && <a href={`/agreements/${a.id}/document`} target="_blank" rel="noreferrer" className="ml-2 text-xs font-normal text-accent hover:underline">PDF</a>}</Td><Td>{labelForCode(a.serviceCode, a.modifiers)}<div className="text-xs text-muted tabular-nums">{a.serviceCode} {a.modifiers.join(" ")}</div></Td><Td><span className="flex items-center gap-2"><Ring used={unitsUsed} total={a.authorizedUnits} size={26} /><span className="tabular-nums">{unitsUsed.toLocaleString()} / {a.authorizedUnits.toLocaleString()}</span></span></Td><Td align="right">{fmtMoney(a.unitRate)}</Td><Td className="text-muted">{fmtDate(a.startDate)} – {fmtDate(a.endDate)}</Td><Td>{a.authorizingCounty}</Td><Td><Badge tone={a.status === "active" ? "ok" : a.status === "cancelled" ? "danger" : "neutral"}>{a.status}</Badge></Td><Td align="right">{manage && <AgreementStatusButton id={a.id} personId={id} status={a.status} />}</Td></Tr>)}</tbody>
            </Table>
          )}
        </Card>
      )}
            ...
            {periods.map((p, i) => {
              ...
              return (
                <div key={p.index} className={cx(i > 0 && "mt-6")}>
                  <div className="mb-3 flex flex-wrap items-baseline justify-between gap-2 rounded-md border border-line bg-sidebar px-3 py-2"><div className="font-medium text-text-strong">{i === 0 ? "Current pay period" : "Pay period"} <span className="font-normal text-muted">· {p.label}</span></div><div className="text-[13px] tabular-nums text-muted">{inPeriod.length} visit{inPeriod.length === 1 ? "" : "s"} · {units} units · {Math.round(minutes / 6) / 10} h</div></div>
                  {days.size === 0 ? <p className="px-3 pb-2 text-[13px] text-muted">No visits in this pay period.</p> : [...days.entries()].map(([day, items]) => (
                    <div key={day} className="relative mb-4 pl-6">
                      ...
                      {items.map(({ visit: v, staffFirst, staffLast, editCount }) => (
                        <Link key={v.id} href={`/visits/${v.id}`} className="mb-2 block rounded-lg border border-line bg-card p-4 shadow-[var(--shadow-sm)] transition-colors hover:bg-hover">
                          <div className="flex flex-wrap items-center gap-x-2 gap-y-1 text-[13px]"><span className="font-medium text-text-strong tabular-nums">{time.format(v.clockInAt)}{v.clockOutAt ? ` – ${time.format(v.clockOutAt)}` : ""}</span><span className="text-muted">· {staffFirst} {staffLast} · {v.serviceCode} · {v.units} unit{v.units === 1 ? "" : "s"}</span><span className="ml-auto flex gap-1"><Badge tone={visitTone(v.status)}>{v.status.replace("_", " ")}</Badge>{v.manualEntry && <Badge tone="warn">manual</Badge>}{editCount > 0 && <Badge tone="warn">edited</Badge>}{v.status === "completed" && !v.clientSignedAt && <Badge tone="danger">unsigned</Badge>}</span></div>
      ...
      {tab === "files" && (
        <Card title="Plans and files" description="Support plan, IAPP, treatment goals, and anything else staff should read before a shift">
          {documents.length === 0 ? <p className="px-5 py-6 text-center text-[13px] text-muted">No files yet. {manage ? "Upload the support plan, the IAPP, and treatment goals below." : "Your supervisor has not uploaded plans for this person yet."}</p> : (
      ...
      {tab === "contacts" && (
        <div className="grid gap-4 md:grid-cols-2 xl:grid-cols-3">
          ...
          <Card title="Care team" padded>{team.length === 0 ? <p className="text-[13px] text-muted">No caregivers assigned. Assign from the staff record.</p> : <ul className="space-y-2">{team.map((t) => <li key={t.assignment.id} className="flex items-center justify-between text-[13px]"><Link href={`/staff/${t.staff.id}`} className="font-medium text-text-strong hover:underline">{t.staff.firstName} {t.staff.lastName}</Link>{t.assignment.orientedOn ? <Badge tone="ok">oriented</Badge> : <Badge tone="warn">orientation pending</Badge>}</li>)}</ul>}</Card>
      ...
      {tab === "history" && user.role === "admin" && (
        <Card title="Record history" description="Every change to this record">
          {audit.length === 0 ? <Empty icon="history" title="No history" /> : <ul className="divide-y divide-line-soft">{audit.map(({ entry, actorEmail }) => <li key={entry.id} className="flex items-center justify-between px-5 py-2.5 text-[13px] text-muted"><span><span className="font-medium text-text">{entry.action}</span> by {actorEmail ?? "system"}</span><span className="tabular-nums">{fmtDateTime(entry.at)}</span></li>)}</ul>}
        </Card>
      )}
      {(null as ReactNode)}
    </div>
