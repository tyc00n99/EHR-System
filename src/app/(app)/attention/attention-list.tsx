...
export function AttentionList({ groups, initialKind = "all" }: { groups: Group[]; initialKind?: string }) {
  ...
  const run = (kind: string) =>
    start(async () => {
      ...
      } catch { toast.error("Could not save changes. Your selection is kept; try again."); }
    });
          ...
          >
            ...
            {bulk && sel.size > 0 && (
              <div className="flex flex-wrap items-center gap-2 border-t border-line-soft bg-sidebar px-5 py-3">
                ...
                {bulk.needsReason && <Input aria-label={bulk.needsReason} value={reason} onChange={(e) => setReason(e.target.value)} placeholder={bulk.needsReason} className="h-8 w-64 text-[13px]" />}
