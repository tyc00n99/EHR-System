...
const CODE_COLOR: Record<string, string> = { H2014: "bg-blue-600", S5135: "bg-violet-600", S5150: "bg-teal-600", S5125: "bg-sky-600", T2021: "bg-amber-600", T2019: "bg-emerald-600", S5130: "bg-pink-600", H2032: "bg-indigo-600" };
        ...
        {days.map((d) => (
          <div key={d} className="relative border-r border-line-soft last:border-r-0">
            ...
            {shifts.filter((s) => s.date === d).map((s) => {
              ...
              return (
                <Link key={s.id} href={`${baseHref}&shift=${s.id}`} scroll={false} className={cx("absolute left-1 right-1 overflow-hidden rounded-md px-2 py-1 text-white shadow-sm ring-1 ring-black/5 hover:brightness-110", CODE_COLOR[s.code] ?? "bg-gray-600", cancelled && "opacity-40 line-through", missed && "ring-2 ring-danger", done && "opacity-80")} style={{ top, height }}>
                  <div className="truncate text-[11px] font-semibold leading-4">{fmt.format(new Date(s.start))} · {s.client}</div>
                  <div className="truncate text-[11px] leading-4 opacity-90">{s.staff}</div>
                  {height > 44 && <div className="truncate text-[10.5px] leading-4 opacity-80">{s.service}</div>}
                </Link>
              );
            })}
          </div>
