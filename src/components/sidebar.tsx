...
export function Sidebar({ role, orgName, attention }: { role: Role; orgName: string; attention: number }) {
  ...
  return (
    <aside className={cx("sticky top-0 hidden h-screen shrink-0 flex-col bg-nav text-nav-text transition-[width] duration-200 md:flex", collapsed ? "w-16" : "w-60")}>
        ...
        {groups.map((g, gi) => {
          ...
          return (
            <div key={gi} className={cx(gi > 0 && "mt-4")}>
              {g.label && !collapsed && <div className="mb-1 px-2.5 text-[10.5px] font-semibold uppercase tracking-[0.08em] text-white/40">{g.label}</div>}
              {g.label && collapsed && <div className="mx-auto mb-2 h-px w-6 bg-white/10" />}
                  ...
                  return (
                    <li key={it.href}>
                      <Link href={it.href} title={collapsed ? it.label : undefined} aria-current={active ? "page" : undefined}
                        className={cx("flex h-9 items-center gap-2.5 rounded-md text-[13.5px] font-medium transition-colors", collapsed ? "justify-center px-0" : "px-2.5", active ? "bg-nav-active text-nav-text-strong" : "text-nav-text hover:bg-nav-hover hover:text-nav-text-strong")}>
                        <span className="relative"><Ic size={17} />{collapsed && it.badge ? <span className="absolute -right-1.5 -top-1.5 h-2 w-2 rounded-full bg-danger" /> : null}</span>
                        {!collapsed && <span className="min-w-0 flex-1 truncate">{it.label}</span>}
                        {!collapsed && it.badge ? <span className="rounded-full bg-danger px-1.5 text-[11px] font-semibold leading-[18px] text-white">{it.badge}</span> : null}
                        {!collapsed && it.soon && <span className="rounded bg-white/10 px-1.5 text-[10px] font-semibold uppercase tracking-wide text-white/60">soon</span>}
                      </Link>
                    </li>
                  );
                })}
      ...
      <button onClick={toggle} className={cx("flex h-11 items-center gap-2 border-t border-white/10 text-[12px] text-nav-text hover:bg-nav-hover hover:text-nav-text-strong", collapsed ? "justify-center" : "px-4")} aria-label={collapsed ? "Expand sidebar" : "Collapse sidebar"}>
