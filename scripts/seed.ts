...
async function main() {
  ...
  const mkGoal = async (title: string, outcome: string, description: string, category: string, prompts: string[], targetDate: string | null = null) => {
    const g = await w.insert(goals, { personId: harold.id, title, outcome, description, supports: description, measurement: "Progress is read from the daily notes and the log under this outcome. Reviewed by the designated coordinator every 90 days and at the annual meeting.", category, status: "active", startDate: "2026-07-01", targetDate, createdBy: adminUser.id });
  ...
  const gMeals = await mkGoal("Eat three meals and drink enough water", "Harold will keep adequate nutrition and hydration by eating three meals a day and drinking at least six glasses of water, so that he stays out of the hospital and keeps the weight he has now (168 lb, June 2026). Harold has forgotten meals since his diagnosis; Ingrid reports he lost 11 pounds between January and May.", "Staff prepare meals at 8:00, 12:30 and 6:00 and sit with Harold while he eats. Offer a choice between two options shown on plates rather than asking open questions. Keep a filled glass within reach and prompt hydration on the hour; note refusals rather than pushing. Weigh on the first Monday of each month, same clothes, before breakfast.", "nutrition", ["Did Harold eat a full meal during this visit?", "Did Harold drink at least two glasses of water?"]);
